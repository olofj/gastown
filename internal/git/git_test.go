...
func TestNonRuntimeFiles(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			s: UncommittedWorkStatus{
				HasUncommittedChanges: true,
				ModifiedFiles:        []string{".claude/settings.json"},
				UntrackedFiles:       []string{".beads/db", ".runtime/state.json", "__pycache__/foo.pyc"},
			},
			want: nil,
		},
		{
		...
		{
			name: "mix filters out runtime",
			s: UncommittedWorkStatus{
				HasUncommittedChanges: true,
				ModifiedFiles:        []string{".claude/mcp.json", "src/main.go"},
				UntrackedFiles:       []string{"new_file.go", ".beads/something"},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, f := range got {
				if f != tt.want[i] {
					t.Errorf("NonRuntimeFiles()[%d] = %q, want %q", i, f, tt.want[i])
				}
			}
		})
	}
